...
class WorldModel:
    ...
    def build_prompt(self, session_id: str, user_message: str, n_turns : int = 5, context: str = "") -> list:
        """
        ...
        for turn in turns:
            messages.append(SystemMessage(content=f"NARRATOR: {turn['narration']}")) # Only add the narrator part (user action not needed )
            rag_query_parts.append(f"NARRATOR: {turn['narration']}")
        
        if context:
            messages.append(SystemMessage(content=f"ADDITIONAL CONTEXT: {context}")) # to add extra context if needed
